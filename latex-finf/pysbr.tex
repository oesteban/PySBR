...
\newglossaryentry{spm}{%
  ...
  first       = {\emph{SPM} (Statistical Parametric Mapping, \cite{ashburner_unified_2005})},%
}

\newglossaryentry{syn}{%
  name        = {SyN},%
  description = {Symmetric Normalization transformation model},%
  first       = {Symmetric Normalization (SyN) transformation model \citep{avants_symmetric_2008}},%
}

\newglossaryentry{ants}{%
  name        = {ANTS},%
  description = {Advanced Normalization tools},%
  first       = {\emph{ANTS} (Advanced Normalization tools, \cite{avants_ants:_2013})},%
}

\newglossaryentry{itk}{%
  name        = {ITK},%
  description = {Insight Registration \& Segmentation Toolkit},%
  first       = {\emph{ITK} (the Insight Registration \& Segmentation Toolkit, \url{http://www.itk.org})},%
}

\newglossaryentry{pysbr}{%
  name        = {PySBR},%
  description = {Shape-based registration in Python},%
  first       = {\emph{PySBR} (shape-based registration in Python)},%
}
...
\newacronym{ibr}{IBR}{intensity-based registration}
...
The second edition of the \emph{Python in Neuroscience} Research Topic,
  to which this paper belongs, is a confirmation of the trends described
  above.
Under this topic, we show how rigorous evaluation pipelines can be designed
...
Some efforts have been made to help automate the process and standardize
  the outcome \citep{calvini_basal_2007,badiavas_spect_2011,ciarmiello_weighted_2013}.
