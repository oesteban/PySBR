...
In yellow, the corresponding contours from the template projected into the subject's space through 
  the deformation field obtained with nonlinear registration, showing the problematic matching
...
Other solutions are implemented adapting the nonlinear registration tools included
  ...
  imposing a strong regularization penalty \fixme{reference needed}.
...
\subsection{Our contribution}\label{sec:our-contrib}
...
In order to demonstrate the utility of the evaluation framework we compare 
  two methods, delivering the source code and data necessary to run the tests.
The first methodology under comparison is one among currently standard approaches,
  that use \gls*{ibr}.
The second method is called \gls*{pysbr}, a new shape based registration tool and
...
\begin{table*}[!t]
  \processtable{\textbf{Configuration of Initialization and Refinement stages using \gls*{ibr}}.
                ...
                Levels 0 and 1 implement to the \emph{Initialization stage}, shared
                for all \emph{Refinement} modules under study.
                Remaining levels correspond to the \emph{Refinement stage} using
  ...
  \hline \\[-0.8em]
        2     & Regular$^{*}$ ($1\times1\times1$) & Cross-correlation metric (``CC'' option in \gls*{ants}) with 6 pixels radius, \glsreset{syn}\gls*{syn}, $\tau_g = 2.0$, $\sigma^2_u = 6.0mm$, $\sigma^2_f = 10.0mm$. Fixed-image masked with ``Search-ROI''. \\
  \hline \\[-0.8em]
        3     & Regular$^{*}$ ($1\times1\times1$) & ``CC'' metric with 1 pixel radius, \gls*{syn} transformation model, $\tau_g = 0.75$, $\sigma^2_u = 4.0mm$, $\sigma^2_f = 6.0mm$. Fixed-image masked with ``Search-ROI''. \\
  \hline \\[-0.8em]
        All   & -                              & Default metric is mutual information (``Mattes''), with joint histogram of 64 bins. Preprocessing filters: histogram matching, intensity window at (15\%, 100\%) percentiles. \\
        \end{tabular} %
        }
