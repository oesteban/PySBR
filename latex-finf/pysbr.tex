...
\newacronym[first={Symmetric Normalization (\emph{SyN}) transformation model \citep{avants_symmetric_2008}}]%
...
\newacronym[first={\emph{BasGan} (\url{http://www.aimn.it/struttura/gruppi/gs_neuro.php}, \cite{calvini_basal_2007})}]%
...
\title[Evaluating normalisation]%
  {Evaluating spatial normalisation in fully automated SPECT imaging quantification protocols}
...
In order to achieve this standardization, the full automation of the
  process requires precise image normalisation to/from a standard space.
...
In this paper, we present a new evaluation framework implemented with \emph{nipype},
  and we propose \gls*{pysbr}, a new spatial normalisation tool implemented in Python for 
...
We use the evaluation tool to study \gls*{pysbr} and a standard normalisation
...
Finally, we compare the proposed tools on the normalisation of 5 real datasets.
...
Quantification of \gls*{datscan} can facilitate early 
  ...
  striatal structures in the \gls*{datscan}
...
Therefore, the key processing step to automate quantification is the
  spatial mapping (or \emph{spatial normalisation}) of the subject's image and
  the template.
...
Other solutions are implemented adapting the nonlinear registration tools included
  with \gls*{spm}, using the \emph{spatial normalisation} module and settings
...
Therefore, we propose an evaluation framework implemented in Python for the
  spatial normalisation of \gls*{datscan} images required by absolute quantification
  protocols.
...
(2) a fast \emph{Initialization stage}, common for all the spatial
    normalisation packages under comparison;
...
On one hand, we use a standard spatial normalisation 
...
All the subjects were normalized to the \gls*{t1} template distributed with
  \gls*{spm}2.
This normalisation step was validated by visual inspection.
...
Third, the \gls*{spect} studies were rigidly registered to their corresponding \gls*{t1}, 
  and then the normalisation transform was applied.
Finally, the resampling, the normalisation of \gls*{spect} counts, and the final
...
\subsection{IBR: intensity-based registration}
  \begin{table*}[!t]
  \processtable{\textbf{Configuration of initialization and \gls*{ibr}}.
                Table presents registration levels and corresponding parameters
                for \Gls*{ants}.
                Levels 0 and 1 implement to the \emph{Initialization} step, shared
                by \gls*{ibr} and \gls*{pysbr}.
                Remaining levels correspond to the \emph{refinement} step using
                a nonlinear \gls*{ibr} algorithm.\label{Tab:01}} %
        {
        \begin{tabular}{llp{11cm}}
        Level & Sub-sampling                   & Parameters configuration     \\
  \hline 
  \hline  \\[-0.8em]
        0     & Regular$^{*}$ ($4\times4\times4$)    & Rigid (6 parameters) transformation, pre-smoothing of $\sigma=8.0mm$. 
                                                 Initialized with a translation mapping geometrical centers of template and subject's images.\\
  \hline \\[-0.8em]
        1     & Regular$^{*}$ ($2\times2\times2$)    & Affine (12 parameters) transformation, pre-smoothing of $\sigma=4.0mm$. \\
  \hline \\[-0.8em]
        2     & Random$^{**}$ (20\% of total samples) & \glsreset{syn}\gls*{syn}, gradient step $2.0$, variance of update field $6.0mm$, variance of total field $10.0mm$. \\
  \hline \\[-0.8em]
        3     & Random$^{**}$ (20\% of total samples) & \gls*{syn}, gradient step $0.75$, variance of update field $4.0mm$, variance of total field $6.0mm$.  \\
  \hline \\[-0.8em]
        All   & -                              & Mutual information metric (``Mattes'' option in \gls*{ants}), with joint histogram of 64 bins. Preprocessing filters: histogram matching, intensity window at (25\%, 99.8\%) percentiles. \\
        \end{tabular} %
        }
        { ($^{*}$) ``Regular'' subsampling stands for decimation by integer factors
           of the image's grid. \\
           ($^{**}$) ``Random'' subsampling randomly draws
           samples from the full grid without geometrical restrictions.}
  \end{table*}
...
\centering
  ...
  registration pattern for spatial normalisation, replacing the intensity-based
...
First, the template data that is used as the basis for the data normalisation.
...
In \autoref{sec:user_interface} we introduce the main \emph{Workflows} that are
  ...
  for normalisation of \gls*{datscan} studies in standardized
...
\subsubsection{Normalisation workflow} %
The core interface for the user is offered by this workflow,
  which implements the normalisation of a given \gls*{spect} 
...
Quantitative results, grouped by grade of the stage of
  syndrome and spatial normalisation methodology, 
  are given in \autoref{fig:results_a}.

\glsreset{ibr}
With the term \gls*{ibr}, we identify the spatial normalisation
...
In fact, the estimated medians of each \gls*{roi} are always negative: i. e. in average
  the normalisation achieved with the initialization was not improved.
...
Within the framework of absolute quantification of \gls*{spect}
  ...
  to evaluate current spatial normalisation techniques that are
...
Spatial normalisation of \gls*{spect} imaging studies is
