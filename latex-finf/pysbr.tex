...
\begin{abstract}
\section{}
Current trends in biomedical imaging invite to reproducible research
  through open databases released together to their validation data
  and promoting image processing challenges with a common framework
  of evaluation.
As a response to this tendency, in this work we propose a
  framework to evaluate spatial normalization methods for fully
  automated quantification of \gls*{spect} images.
This task remains an open challenge in the diagnosis and stratification
  of motion disorder patients, and it requires an accurate and reliable
  spatial normalization for proper quantification.
The spatial normalization is usually performed in two steps: an
  \emph{initialization} step that finds a global and linear matching
  between the subject's image and a template; and the \emph{refinement}
  step that locally improves the alignment of certain \acrlongpl*{roi}
  through a nonlinear registration.

To show the usefulness of the proposed evaluation framework, we benchmarked
  two approaches to spatial normalization.
First, we adapted a standard \acrlong{ibr} tool to perform the nonlinear
  mapping.
Second, we tested a novel methodology based on detected shape-features,
  called \gls*{pysbr}.
The \acrlong{ibr} behaved consistently, with small --when
  present-- accuracy improvements after the refinement stage.
Quantitative and qualitative results illustrate that our evaluation
  framework successfully ranked both algorithms.
Additionally we aimed to show how this type of frameworks are useful to
  design new methods for specific tasks.
In this sense, it helped identify the valuable features of \gls*{pysbr},
  and the necessity of improved landmark detection to increase
  its reliability.
\tiny
\section{Keywords:}  spect, nonlinear registration, quantification, movement disorders,
...
Currently, our \emph{in-house} approach is implemented adapting the nonlinear registration
  tools included with \gls*{spm}, likewise in \citep{rakshi_frontal_1999}.
...
Spatial normalization of \gls*{spect} imaging studies can be
  performed semi-automatically by means of \gls*{ibr} techniques in 
  two steps.
\cbend
...
\ifthenelse{\boolean{submit}}{
  \newpage
  \setcounter{page}{1}
  \renewcommand*{\thepage}{Figures - \arabic{page}}
  \setcounter{figure}{0} % Start again
