...
\fixme{pg5. ‘In order to produce a trustful benchmarking framework, …’: are those gold standard for general/universal use? or are those designed to use for your evaluation? what do you mean ‘trustful’?}
...
\fixme{revise use of ``cohort''}
\fixme{pg3: ‘we generated very simplistic SPECT-like images mimicking disease in four stages of severity’: This is not clear how SPECT-like images are created.}
...
\fixme{pg4: Section 2.1.4 Templates: What is template? Can you define more precisely? Are you using atlas and template interchangeably? or is the template a set of images including ROIs and landmarks?}
...
\fixme{pg5. ‘… to compare the standard methodologies with our tool.’: what is the standard methodologies? the ANTs is the standard in the field?}
\fixme{pg4. Section 2.2.2: Terms are confusing: Are three terminologies of Feature= Shape Feature = Landmarks indicate identical ‘feature’?}
...
\fixme{We propose a multi-resolution scheme of two levels (identified as 0 and 1) and 
  mutual information as cost function; the detailed parameter description 
  is given in \autoref{Tab:01}. \#11}
...
\fixme{As all the affine mismatch between template and subject should be captured by the initialization
  step, we define a number of \emph{static} landmarks (\lm{static}). (Re-write in understandable English)}
...
\fixme{pg4: this feature extraction results…’ → this shape feature extraction results…?}
...
\fixme{pg5: ‘In order to correct for location…’ : What is wrong for location and orientation??? Why do we need a correction?}
In order to correct for location and orientation of the segmented nuclei, we
  ...
  landmark \fixme{pg5. ‘the primary landmark’: what is the primary landmark???}, obtaining the ``$(r,\theta,\phi)$-plot'' or 3D ``boundary-profile''
